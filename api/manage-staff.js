...
function kaydedilemedi(res) {
  if (sonMesgul) return mesgulYanit(res);
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      if (action === "ekle") {
        ...
        if (!(await hesaplariYaz(alanAdi, guncel))) return kaydedilemedi(res);
