...
export const FINANS_SEKME_TANIMLARI = [
  ...
  /* ÖNÜMÜZDEKİ AYLAR — bütün para ekranları GERİYE bakıyordu. Markanın `bitisAyi`
   * alanı (`lib/marka-donemi.js`) ileriye bakmayı mümkün kıldı; hesap
   * `lib/mali-tahmin.js`'te. Kapısı `finans`: gelecek ay tahmini ödeme kaydı değil,
   * şirketin gelir–gider tablosudur — `odemeTakvimi` izni bunu AÇMAZ. */
  { key: "tahmin", label: "Önümüzdeki Aylar", izin: "finans" },
  /* ÖDEMELER = eski "Ödeme Takvimi" ekranı, sekme hâlinde. Kapısı `odemeTakvimi`.
   *
   * BU KAPI YALNIZCA EKRANI KAPATIYOR, VERİYİ DEĞİL — ölçüldü ve kabul edildi:
   * `PERMISSION_DATA_FIELDS.odemeTakvimi`'nin açtığı dört alanın (`clients`,
   * `hesaplar`, `hesapTransferleri`, `hesapDuzeltmeleri`) HEPSİ `finans` izninin de
   * içinde. Yani `finans` izni olan biri sekmeyi görmese bile ödeme kayıtlarını
   * tarayıcısına alıyor.
   *
   * Bilerek böyle bırakıldı: `finans` izni zaten `personel`i (maaşlar) açıyor, yani
   * ödeme kayıtları o kişinin görebildiklerinin en hassası değil. Sunucuda gerçek
   * bir ayrım kurmak `clients`ı alan alan süzmeyi gerektirir ve `musteriler`,
   * `dashboard`, `takvim` izinlerini de etkiler — kazancına değmiyor.
   *
   * ASIL DARALTILMASI GEREKEN TERS YÖN: `odemeTakvimi` `FULL_CLIENT_PERMS` içinde
   * (`api/data.js:176`), bu yüzden yalnızca ödeme takibi yapsın diye izin verilen
   * kişi müşteri kaydının TAMAMINI — `aylikUcret` ve `maliyetler` dahil — alıyor.
   * Kayıtlı borç; ayrı iş. */
