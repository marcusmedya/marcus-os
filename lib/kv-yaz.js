...
import { kayitliYanit, yanitiSakla } from "./islem-kimligi.js";
...
/* Üst sınır ŞART: her deneme ~250ms bekliyor. Yanlışlıkla büyük bir değer girilirse
 * fonksiyon Vercel'in süre sınırını aşıp isteği tamamen düşürürdü. */
const KILIT_DENEME = Math.min(40, Math.max(1, Number(process.env.KILIT_DENEME) || 12));
 ...
 */
/**
 * BELGE OKUNABİLİR HALDE Mİ?
 *
 * `kv.get` her zaman bir belge döndürmüyor. Anahtar bozulmuşsa, elle başka bir değer
 * yazılmışsa ya da yarım bir yazma kalmışsa geriye metin, dizi ya da sayı gelebiliyor.
 *
 * Eskiden bu durumda `(await kv.get(KEY)) || {}` boş bir belge üretiyordu ve sistem
 * hiçbir şey söylemeden ÇALIŞMAYA DEVAM EDİYORDU: kullanıcı bomboş bir uygulama
 * görüyor, "her şey silinmiş" sanıp yeni kayıt giriyor ve o kayıt bozuk verinin
 * ÜSTÜNE yazılıyordu. Bozuk veri hâlâ kurtarılabilir olabilirdi; üstüne yazmak onu
 * kalıcı olarak yok eder.
 *
 * Kural: VERİ KAYBETMEKTENSE İŞLEMİ REDDET. Yönetici yedekten dönebilir — yedek
 * sistemi tam olarak bunun için var.
 *
 * `null`/`undefined` bozuk DEĞİLDİR: kurulum anında belge henüz yoktur ve boş bir
 * belgeyle başlamak doğru davranıştır.
 */
export function belgeOkunabilirMi(deger) {
  if (deger === null || deger === undefined) return true;      // ilk kurulum
  return typeof deger === "object" && !Array.isArray(deger);
}

export const BOZUK_KOD = 409;
export const BOZUK_MESAJI =
  "Veritabanındaki belge okunamıyor (beklenen yapıda değil). Hiçbir şey yazılmadı — " +
  "veri kaybetmemek için işlem reddedildi. Ayarlar > Yedekler'den son sağlam yedeğe dönebilirsin.";

export async function guvenliGuncelle(degistir, secenekler) {
  /* İŞLEM KİMLİĞİ (opsiyonel).
   *
   * Verilmezse hiçbir şey değişmez — bugünkü davranış birebir sürüyor. Verilirse aynı
   * işlemin ikinci kez uygulanması engellenir.
   *
   * KONTROL KİLİDİN İÇİNDE: dışarıda yapılsaydı iki hızlı tıklama aynı anda "görmedim"
   * cevabı alır ve ikisi de uygulanırdı — korumanın hiç olmaması gibi. */
  const islemId = (secenekler && secenekler.islemId) || null;
   ...
   *
   * KİMLİK DE YAZILMIYOR: yazılsaydı tarayıcının otomatik tekrarı "bunu zaten yaptım"
   * sanılır ve işlem sessizce kaybolurdu. */
  if (!kilitAlindi) {
  ...
  try {
    if (islemId) {
      ...
      if (kayitli) {
        /* Bu işlem daha önce uygulandı. Değiştirme fonksiyonu HİÇ çalıştırılmıyor.
         * `tekrarlandi` ile bildiriliyor ki çağıran taraf yan etkileri (e-posta,
         * bildirim) ikinci kez tetiklemesin. */
        return {
          ok: true, tekrarlandi: true,
          ek: kayitli.yanit && kayitli.yanit.ek,
          veri: (await kv.get(KEY)) || {},
        };
      }
    }
    const ham = await kv.get(KEY);
    /* BOZUK BELGENİN ÜSTÜNE YAZILMAZ. Kontrol kilidin İÇİNDE: dışarıda yapılsaydı
     * arada başka bir yazma belgeyi düzeltmiş ya da bozmuş olabilirdi. */
    if (!belgeOkunabilirMi(ham)) {
      return { ok: false, iptal: true, bozuk: true, kod: BOZUK_KOD, hata: BOZUK_MESAJI };
    }
    const mevcut = ham || {};
    ...
    /* Kimlik YAZMADAN SONRA işaretleniyor — sırası önemli. Önce işaretlenip sonra yazma
     * patlasaydı, işlem hiç uygulanmadığı hâlde "yapıldı" sayılırdı. */
    if (islemId) await yanitiSakla(islemId, 200, { ek: sonuc.ek });
