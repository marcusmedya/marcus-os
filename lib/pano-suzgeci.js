 ...
 * @param kategoriler tanınan kategori listesi
 */
export function panoSuzgeci(isler, kategori, marka, kategoriler) {
  const taninan = Array.isArray(kategoriler) ? kategoriler : [];
  /* ESKİ KATEGORİ ADLARI EŞLEMEDEN GEÇİYOR. Belgede "Video", "Fotoğraf", "Grafik
   * Tasarım" kategorili yüzlerce kart var; ham adla karşılaştırılsalardı HİÇBİR
   * SEKMEDE görünmezlerdi — aylardır süren işler panodan kaybolurdu. Ölçüldü. */
  const hedef = kategoriEsle(kategori);
  const kategoriIsleri = (isler || []).filter((j) => {
    if (!j) return false;
    const kendi = kategoriEsle(j.kategori);
    return taninan.length === 0 ? kendi === hedef : kendi === hedef;
  });
