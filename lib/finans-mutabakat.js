...
export function finansMutabakati(data, hareketler, secenekler = {}) {
  ...
  const satirlar = [];
  const sebepler = [];
  /* BİRİM SATIRIN KENDİSİNDE TAŞINIR — ekran onu ADINDAN tahmin etmesin.
   *
   * Satırların çoğu para, ikisi ADET ("Tarihsiz tahsilat kaydı", "Üyelik kaydı"). Ekran
   * bunu ayırt edemezse sayıyı ₺ ile biçimlendirip "₺3" yazar. Ayrımı JSX'te `ad.includes(
   * "(adet)")` diye yapmak, başlık metni değişince SESSİZCE kopan bir kural olurdu
   * (`marcus-mimari` §4). Kural burada, modülün kendisinde duruyor ve sınanabiliyor. */
  const satir = (ad, eski, yeni, birim = "tl") => {
    const e = yuvarla(eski);
    const y = yuvarla(yeni);
    satirlar.push({ ad, eski: e, yeni: y, fark: yuvarla(y - e), birim });
  };
  ...
  satir("Tarihsiz tahsilat kaydı (adet)", eskiTahsilat.tarihsiz,
    liste.filter((h) => h.tur === "tahsilat" && !h.donem).length, "adet");
  ...
  satir("Tarihsiz ödeme kaydı (adet)", eskiOdeme.tarihsiz,
    liste.filter((h) => h.tur === "odeme" && !h.donem).length, "adet");
  ...
  satir("Üyelik kaydı (adet)", dizi(belge.uyelikler).length,
    liste.filter((h) => h.kaynakAlan === "uyelikler").length, "adet");
