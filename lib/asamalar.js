...
export const ASAMALAR = ASAMALAR_REELS;

const ASAMA_TABLOSU = {
  "Reels": ASAMALAR_REELS,
  "Post": ASAMALAR_POST,
  "Carousel": ASAMALAR_CAROUSEL,
};


export const asamaListesi = (kategori) => ASAMA_TABLOSU[kategoriEsle(kategori)] || ASAMALAR_REELS;
...
const YAPILIYOR_TABLOSU = {
  "Reels": "Edit Yapılıyor",
  "Post": "Düzenleniyor",
  "Carousel": "Düzenleniyor",
};

/** Kategorisiz eski kayıtlar Video akışını kullanır. */
/** Kartın "üzerinde çalışılıyor" aşaması. Eski kategori adları eşlemeden geçiyor. */
export const yapiliyorAsamasi = (kategori) => YAPILIYOR_TABLOSU[kategoriEsle(kategori)] || YAPILIYOR_TABLOSU.Reels;
...
/* ESKİ TASARIM AŞAMASI → POST KARŞILIĞI.
 *
 * Grafik Tasarım kategorisi kalktı, kartları Post'a katıldı. "Tasarım Yapılıyor" Post
 * listesinde yok; eşlenmeseydi `asamalariDuzelt` kartı listenin BAŞINA çeker ve üzerinde
 * çalışılan bir iş yeniden kuyruğa girerdi.
 *
 * YALNIZCA BU AŞAMA EŞLENİYOR. Bir süre "Talep Alındı" ve "Tasarım Bekliyor" da
 * "Çekim Yapıldı"ya eşleniyordu — ölçüldü: o zaman "Talep Alındı" taşıyan bir REELS
 * kartı da oraya düşüyor, yani YAPILMAMIŞ bir çekim yapılmış sayılıyordu. O iki aşama
 * eşlenmeden bırakılınca kart kendi kategorisinin ilk aşamasına iniyor — doğrusu bu.
 */
export const ESKI_TASARIM_ASAMALARI = {
  "Tasarım Yapılıyor": "Düzenleniyor",
};
 ...
 *
 * İKİ ADIM SIRAYLA: önce kaldırılmış ara aşamalar ("Tasarım Yapıldı" → "Tasarım
 * Yapılıyor"), sonra kalkan Grafik Tasarım akışının Post karşılıkları ("Tasarım
 * Yapılıyor" → "Düzenleniyor"). Tek adım olsaydı "Tasarım Yapıldı" ilk çeviriden sonra
 * yine Post listesinde olmayan bir ada düşer ve kart akışın BAŞINA çekilirdi. */
export const asamaKarsiligi = (asama) => {
  const bir = KALDIRILAN_ASAMALAR[asama] || asama;
  return ESKI_TASARIM_ASAMALARI[bir] || bir;
};
...
/* Bir karta eklenebilecek en fazla slayt. Karosel gönderilerde kullanılıyor; Post tek
 * görsellik olduğu için bu sınıra girmiyor (bkz. `enFazlaSlayt`).
 *
 * 10'du, kullanıcı isteğiyle 30'a çıkarıldı. Sunucu doğrulaması (`slotGecerliMi`),
 * boş slot bulma (`bosSlot`) ve arayüzdeki uyarı metni hep bu sabite bakıyor — sayı
 * tek yerde durduğu için üçü ayrışamaz. */
export const EN_FAZLA_SLAYT = 30;
...
export function enFazlaSlayt(kategori) {
  /* POST TEK GÖRSELLİK. Çoklu görsel Carousel'in işi — ikisi de çoklu olduğunda
   * "kaydırmalı gönderi mi, ayrı postlar mı" ayrımı kayboluyor ve stok yanlış türe
   * yazılıyordu. Sınır yalnızca YENİ yuva açmaya uygulanır; eski çok slaytlı kartların
   * dosyaları görünmeye devam eder. */
  return kategoriEsle(kategori) === "Post" ? 1 : EN_FAZLA_SLAYT;
}
...
/**
 * PANO KARTINDA ÖNİZLEME GÖSTERİLSİN Mİ?
 *
 * Koşul bir süre doğrudan JSX içinde `job.editliDosyaLink &&` diye yazılıydı: yalnızca
 * ELLE YAPIŞTIRILMIŞ bağlantısı olan kartta önizleme çıkıyor, uygulamadan yüklenen dosya
 * `medya[]` içinde durduğu için o kartlar panoda BOŞ görünüyordu. Toplu açılan yirmi kartta
 * hepsi birden fark edildi. Görüntüyü zaten sunucu veriyor ve sunucu dosyayı `medya`dan da
 * çözüyor (t52 ölçüyor) — eksik olan tek şey bu koşuldu.
 *
 * Kural buraya TAŞINDI çünkü JSX içindeki bir koşul Node'da çağrılamıyor, yalnızca kaynak
 * metnine bakılabiliyordu. Bu projede aynı sınıftan dört hata çıktı.
 *
 * Reels'te önizleme yok: çıktı video, kartta gösterilecek durağan bir kare üretilmiyor.
 */
export function panoOnizlemesiVarMi(is) {
  if (!is) return false;
  if (kategoriEsle(is.kategori || "Reels") === "Reels") return false;
  return medyaVarMi(is);
}
  ...
  const duzeltilmis = (sonrakiIsler || []).map((j) => {
    ...
    return {
      ...
      gecmis: [...(j.gecmis || []), {
        ...
        tarih: zaman, zaman: new Date().toISOString(),
        yazan: "Sistem",
