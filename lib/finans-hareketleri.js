...
/**
 * EKSİK BİLGİNİN SAYIM ÖZETİ — ekranda YORUMLANMAK ÜZERE.
 *
 * `hareketYap` her kaydın neyini bilmediğini `eksikBilgi` dizisine yazıyor, ama o bilgi
 * kayıt kayıt duruyor: "on iki kaydın tarihi yok" sorusunun cevabı hiçbir yerde toplu
 * değildi. Sayımı arayüzde yapmak `marcus-mimari` §4'ün yasakladığı şey olurdu — JSX'e
 * gömülen kural Node'dan çağrılamaz, yani sınanamaz.
 *
 * YENİ BİR KURAL ÜRETMİYOR: yalnızca var olan `eksikBilgi` etiketlerini sayıyor. Hiçbir
 * rakam hesaplanmıyor, hiçbir kayıt elenmiyor.
 *
 * SAF: girdi değiştirilmez, `Date` yok, yan etki yok.
 *
 * @param hareketler `finansHareketleri(...).hareketler`
 * @returns { toplam, tarihsiz, kdvBilinmeyen, stopajBilinmeyen, tutarBilinmeyen,
 *            sirayaBagliKimlik, eksigiOlan }
 */
export function eksikBilgiOzeti(hareketler) {
  const liste = dizi(hareketler).filter(Boolean);
  const say = (etiket) => liste.reduce(
    (s, h) => s + (Array.isArray(h.eksikBilgi) && h.eksikBilgi.includes(etiket) ? 1 : 0), 0);
  return {
    toplam: liste.length,
    /* `donem` üzerinden sayılıyor, `eksikBilgi` üzerinden DEĞİL: ikisi aynı şeyi söylese
     * de dönem alanı asıl kaynak — `finansHareketleri`'nin `tarihsiz` sayacı da onu
     * kullanıyor ve iki rakamın ayrışması tam olarak önlenmek istenen şey. */
    tarihsiz: liste.reduce((s, h) => s + (h.donem ? 0 : 1), 0),
    kdvBilinmeyen: say("kdv"),
    stopajBilinmeyen: say("stopaj"),
    tutarBilinmeyen: say("tutar"),
    /* Kaydın kendi `id`si yok; kimliği listedeki SIRASINA bağlı ve liste değişirse kayar. */
    sirayaBagliKimlik: say("kaynakId"),
    eksigiOlan: liste.reduce(
      (s, h) => s + (Array.isArray(h.eksikBilgi) && h.eksikBilgi.length > 0 ? 1 : 0), 0),
  };
}
