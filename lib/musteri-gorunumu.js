import { etkinAltMetin } from "./alt-yazi.js";
...
export const MUSTERI_ASAMA_DURUM = {
  ...
  "Onaylandı": "onaylandi",
  /* Çok şubeli markada içerik bazı şubelerde paylaşıldı, bazıları bekliyor. Müşteri
   * açısından fark yok: içerik onaylı ve yayında. Eşleme olmasaydı kart müşteri
   * panelinden sessizce kaybolurdu. */
  "Şubelerde Paylaşılıyor": "onaylandi",
...
export function musteriGorunumuUret(data, kendiMarka, markaEslestirici) {
  ...
  const kendiPaylasimPlani = (data.haftalikPaylasimlar || [])
    ...
    .map((p) => ({
      ...
      /* ALT YAZI DEVRALINIYOR. Metin KARTIN özelliği; plan kendi metnini yazmamışsa
       * kartınki gösteriliyor. Burada çözülmezse kartta yazılan metin müşteriye HİÇ
       * ulaşmazdı — özellik müşteri tarafında çalışmıyor görünürdü. */
      altMetin: etkinAltMetin(p, (data.cekimIsleri || []).find((j) => String(j.id) === String(p.isId))) || null,
      ...
      isId: p.isId || null,
      isAdi: p.isAdi || null,
      /* ŞUBE. Kayıtta zaten kopyalı duruyor ve müşterinin KENDİ markasının şubesi —
       * dışarı bilgi sızdırmıyor. Panelde aynı içeriğin dört kez tekrar etmesini
       * engelleyen gruplama buna bakıyor. */
      subeId: p.subeId != null ? String(p.subeId) : null,
      subeAdi: p.subeAdi || null,
    }));
