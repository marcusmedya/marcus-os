...
export function paylasimTuru(is) {
   ...
   * Grafik Tasarım'ınki tasarımdır.
   *
   * VİDEO KATEGORİSİ → REELS. Bu bilerek böyle: ajansın çektiği video içerik pratikte
   * Reels olarak paylaşılıyor. Eskiden "Video" stoğuna düşüyordu ve sonuç şuydu — adında
   * "Reels" geçen kart Reels'e, geçmeyen aynı işteki kart Video'ya yazılıyordu. İki kart
   * arasındaki tek fark, birinin adına o kelimenin yazılmış olmasıydı; kullanıcı sahada
   * bunu gördü. "Video" stoğu artık YALNIZCA kartta açıkça seçildiğinde kullanılıyor. */
  const kategori = trKucult(is.kategori);
  if (kategori === "video") return "Reels";
