 ...
 */
/* Operasyon alt yetkileri (`kartAcma`, `kartOnaylama`, `kartDuzenleme`, `kartSilme`)
 * bu listede OLMAK ZORUNDA. Bu işlev listede olmayan her izni sıfırlıyor; alt yetkiler
 * dışarıda kalsaydı marka kilitli hesap `cekimEdit` açık olduğu hâlde kart AÇAMAZ hâle
 * gelirdi — bugün açabiliyor. Bunlar marka geneli veri değil, kartın kendi işlemleri;
 * hangi kartı gördüğünü zaten marka kilidi belirliyor. */
export const KILITLI_IZINLER = ["paylasimlar", "cekimListesi", "cekimEdit", "reklamlar", "uyelikler", "sifreKasasi", "takvim", "musteriAkisi",
  "kartAcma", "kartOnaylama", "kartDuzenleme", "kartSilme"];
...
const DIZI_ALANLARI = [
  ...
  { ad: "musteriTalepleri", client: "clientId" },
...
/**
 * BİRLEŞTİRMEDE DÜŞEN KAYITLAR — sessiz kayıp yerine açık uyarı.
 *
 * `yazmayiBirlestir` kilitli hesabın gönderdiği kayıtlardan yetkisi olmayanları eler.
 * Elemek doğru; SESSİZCE elemek değil. Kullanıcı kartı ekranında görmeye devam ediyor,
 * sunucuda yok: o karta dosya yüklemeye çalıştığında "İş kartı bulunamadı" alıyor ve
 * ilk tazelemede kart ekrandan da kayboluyor — hiçbir aşamada bir hata görmeden.
 *
 * İki gerçek durum var:
 *   - Kart hiç markasız oluşturuldu (kilitli hesapta bu kaydı sahipsiz bırakır).
 *   - Kart bu hesabın yetkili olmadığı bir markaya ait.
 * İkisinde de kullanıcı SEBEBİ görmeli.
 */
export function birlestirmedeDusenler(mevcut, gonderilen, alan, izinliMarkalar) {
  const setler = izinliIdSeti(mevcut || {}, izinliMarkalar);
  if (!setler) return [];
  const tanim = DIZI_ALANLARI.find((t) => t.ad === alan);
  if (!tanim || !Array.isArray(gonderilen[alan])) return [];
  return gonderilen[alan]
    .filter((k) => !kayitIzinliMi(k, tanim, setler))
    .map((k) => ({
      id: k && k.id,
      marka: (k && (k[tanim.marka] || k[tanim.client])) || "",
      sebep: (k && tanim.marka && !k[tanim.marka]) ? "markasiz" : "yetkisiz",
    }));
}
...
/**
 * MARKA ADINDAN MARKA KİMLİĞİ.
 *
 * İş kartları markayı ADIYLA saklıyor (`job.marka`), şubeler ise `clientId` ile
 * bağlı. Şube kurallarının çalışabilmesi için bu iki dünyanın birbirine
 * çevrilmesi gerekiyor. Çeviriyi her ekranın kendi başına yapması, aynı markanın
 * bir ekranda tanınıp diğerinde tanınmaması demek — o yüzden tek yer burası.
 *
 * Eşleştirme `markaEslestirici` üzerinden: aynı ada sahip iki marka varsa
 * belirsizliğe düşmez, yalnızca birebir yazım eşleşir.
 *
 * @returns marka kimliği, bulunamazsa null
 */
export function markaninIdsi(clients, markaAdi) {
  if (!markaAdi) return null;
  const bulunan = (clients || []).find((c) => c && markaEslestirici(clients, c.ad)(markaAdi));
