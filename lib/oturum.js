 ...
 *
 * DIŞA AÇIK (davranış DEĞİŞMEDİ, yalnızca `export` eklendi): oturumu düşen kullanıcıya
 * gösterilen mesaj bu süreleri yazıyor (`lib/istek-hatasi.js`). O modül SAF olmak
 * zorunda ve burayı import edemez (bu dosya `@vercel/kv` + `crypto` çekiyor, tarayıcıda
 * patlar). Sürelerin iki yerde ayrışmasını `testler/t117.mjs` ölçüyor: burada değişip
 * orada değişmezse test gürültülü kırılır. */
export const SURE_NORMAL = 60 * 60 * 12;
export const SURE_HATIRLA = 60 * 60 * 24 * 30;
