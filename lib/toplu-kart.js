...
/**
 * Başlangıç numarası: elle verilmişse O, verilmemişse markanın serisinin devamı.
 *
 * Otomatik devam çoğu zaman doğru olan ama HER ZAMAN doğru olmayan bir varsayım:
 * kullanıcı eski bir seriye dönmek, ayrı bir aralık açmak (101'den başlatmak) ya da
 * silinmiş kartların yerini doldurmak isteyebiliyor. Bu yüzden değer önerilir, DAYATILMAZ.
 * Geçersiz giriş (0, eksi, metin) otomatiğe düşer — kullanıcının yazarken sildiği bir
 * hane yüzünden "Post 0" açılmasın.
 */
export function baslangiciCoz(mevcutAdlar, taban, baslangic) {
  if (baslangic !== undefined && baslangic !== null && String(baslangic).trim() !== "") {
    const n = Math.floor(Number(baslangic));
    if (Number.isFinite(n) && n >= 1) return n;
  }
  return sonrakiNumara(mevcutAdlar, taban);
}
 ...
 *
 * `baslangic` verilmezse numara markanın serisinden devam eder.
 */
export function topluAdlar(taban, adet, mevcutAdlar, baslangic) {
  const t = tabanTemizle(taban);
  const n = adetiCoz(adet);
  if (!t || n === 0) return [];
  const bas = baslangiciCoz(mevcutAdlar, t, baslangic);
  const liste = [];
...
/**
 * Üretilecek adlardan hangileri markada ZATEN VAR.
 *
 * Başlangıç elle seçilebildiği için çakışma artık mümkün. Engellemek yerine SÖYLENİYOR:
 * kullanıcı bilerek aynı ada ikinci bir kart açmak isteyebilir (aynı içeriğin ikinci
 * versiyonu gibi), ama bunu bilmeden yapmamalı — aynı ad Drive'da dosya adına ve müşteri
 * paneline gidiyor, iki kayıt ayırt edilemez hâle geliyor.
 */
export function cakisanAdlar(adlar, mevcutAdlar) {
  const varOlan = new Set((Array.isArray(mevcutAdlar) ? mevcutAdlar : []).map(trKucult));
  return (Array.isArray(adlar) ? adlar : []).filter((ad) => varOlan.has(trKucult(ad)));
}
 ...
 */
export function topluIsleriUret({ taban, adet, mevcutAdlar, ortak, topluId, baslangic }) {
  const adlar = topluAdlar(taban, adet, mevcutAdlar, baslangic);
