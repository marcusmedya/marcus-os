...
/* ADRESİN KARARLI KALDIĞI PENCERE.
 *
 * Bitiş zamanı jetonun İÇİNDE ve imzalanıyor; saniye saniye değişince her açılışta
 * bambaşka bir adres üretiliyordu. Adres değişince tarayıcı önbelleği de değişiyor:
 * aynı videoyu ikinci kez açmak, ilk kez açmakla birebir aynı maliyetteydi — hiçbir
 * parça yeniden kullanılmıyordu.
 *
 * Bitiş zamanı bir saatlik ızgaraya oturtuluyor: aynı saat içinde aynı kayıt için
 * üretilen jetonlar BİREBİR AYNI, dolayısıyla adres de aynı ve indirilmiş parçalar
 * yeniden kullanılıyor.
 *
 * ÖMÜR UZAMIYOR: ızgara `simdi`nin TABANINA oturtulduğu için jeton en fazla OMUR_SN
 * yaşıyor (pencerenin başında üretilirse), en az OMUR_SN - PENCERE_SN (pencerenin
 * sonunda). Yani üst sınır eskisiyle aynı, alt sınır bir saat — bir videoyu izlemeye
 * fazlasıyla yeter. Yukarı yuvarlansaydı ömür üç saate çıkardı. */
const PENCERE_SN = 60 * 60;
...
export function jetonUret(tur, kimlik, simdi = Date.now(), dosyaId = null) {
  ...
  const biter = Math.floor(Math.floor(simdi / 1000) / PENCERE_SN) * PENCERE_SN + OMUR_SN;
...
/* ================================================================
 * ARALIK PARÇALAMA — tek yanıtta tüm dosyayı akıtma.
 *
 * Tarayıcı videoyu açarken `Range: bytes=0-` diyor: "buradan sonuna kadar". Aynen
 * iletilirse fonksiyon dosyanın TAMAMINI tek yanıtta akıtmaya çalışıyor. İki sert
 * sınıra çarpıyor:
 *   - Fonksiyonun çalışma süresi sınırlı (60 sn). Uzun bir akış ortasından kesiliyor,
 *     video DONUYOR — izlerken yaşanan tam olarak bu.
 *   - Büyük gövdeler sunucusuz yanıt sınırlarına takılıyor.
 *
 * Çözüm: her istek sınırlı bir parça döndürüyor. Tarayıcı kaldığı yerden yeni bir aralık
 * istiyor — normal bir dosya sunucusu da böyle davranır. Her parça saniyeler içinde
 * bitiyor, kesilme noktası kalmıyor.
 *
 * PARÇA BOYU — ÖLÇÜLEREK BÜYÜTÜLDÜ. Önce 3 MB seçilmişti; sahada belirti şuydu: video
 * başlıyor, 1-2 saniye sonra DONUYOR, bir süre sonra devam ediyor. Sebep buydu — yüksek
 * bit hızlı bir Reels'te 3 MB ancak birkaç saniye, her parça bitişinde yeni bir tur
 * atılıyor ve donma tam orada yaşanıyor.
 *
 * 12 MB, aynı videoda on saniyeye yakın görüntü: tarayıcı arabelleği tükenmeden bir
 * sonraki parçayı isteyebiliyor. Süre sınırı hâlâ uzakta — 200 KB/s gibi kötü bir hatta
 * bile 60 saniyenin altında iniyor.
 *
 * SONDAN aralık (`bytes=-500`) ve zaten küçük aralıklar OLDUĞU GİBİ geçiyor —
 * daraltmanın anlamı yok, üstelik sondan aralığın başlangıcı sunucuda bilinmiyor.
 * ================================================================ */
export const VIDEO_PARCA_BAYT = 12 * 1024 * 1024;

/** İstenen aralığı en fazla bir parça olacak şekilde daraltır. */
export function aralikDarailt(aralik, parca = VIDEO_PARCA_BAYT) {
  if (!aralik) return `bytes=0-${parca - 1}`;
  const m = /^bytes=(\d+)-(\d*)$/.exec(String(aralik).trim());
  if (!m) return aralik;                       // sondan aralık ya da tanınmayan biçim
  const bas = Number(m[1]);
  const istenenSon = m[2] === "" ? Infinity : Number(m[2]);
  const son = Math.min(istenenSon, bas + parca - 1);
  return `bytes=${bas}-${son}`;
}
