import { hedefKlasoruHazirla, servisHesabinaYetkiVer, DURUM_KLASORLERI } from "./drive-tasima.js";
 ...
 */
export function dosyaAdiUret({ marka, icerikAdi, versiyon, orijinalAd, mimeTur }) {
  const parcalar = [sadelestir(marka), sadelestir(icerikAdi)].filter(Boolean);
  const govde = parcalar.join("_") || "ICERIK";
  return `${govde}_V${Number(versiyon) || 1}.${uzanti(orijinalAd, mimeTur)}`;
}
 ...
 */
export async function yuklemeOturumuAc({ markaKlasoru, markaAdi, icerikAdi, versiyon, orijinalAd, mimeTur, boyut, origin }) {
  if (!yuklemeHazirMi()) return { ok: false, sebep: "Drive yükleme kurulu değil." };

  const klasor = await hedefKlasoruHazirla({
    markaKlasoru, markaAdi, durumAdi: DURUM_KLASORLERI.onayBekleyen,
  });
  if (!klasor.ok) return { ok: false, sebep: klasor.sebep };

  const ad = dosyaAdiUret({ marka: markaAdi, icerikAdi, versiyon, orijinalAd, mimeTur });
 ...
 */
export async function kucukResimGetir(dosyaId) {
  if (!yuklemeHazirMi()) return { ok: false, sebep: "Drive yükleme kurulu değil." };
  try {
    const jeton = await oauthErisimJetonu();
    const r = await getir(
    ...
    const adres = String(bilgi.thumbnailLink).replace(/=s\d+$/, "=s220");
    ...
    return { ok: true, veri: `data:${tur};base64,${bayt.toString("base64")}` };
  } catch (e) {
    return { ok: false, sebep: String(e.message || e) };
  }
}
