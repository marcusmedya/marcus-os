import { yanittanSebep } from "./eposta-hata.js";
...
export async function epostaGonderAyrintili(to, subject, html, cc) {
  ...
  try {
    ...
    /* Sebep/kod kuralı `lib/eposta-hata.js`'e taşındı — api/daily-backup.js kendi fetch'ini
     * yaptığı için buradaki gömülü kurala erişemiyordu ve sebebi ekrana hiç yazamıyordu. */
    const { sebep, kod } = yanittanSebep(hata, r.status);
    return { ok: false, sebep, kod, gonderen, durum: r.status };
  } catch (e) {
