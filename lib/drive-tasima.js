...
/** Türkçe'ye duyarlı küçültme — "AĞUSTOS" ile "Ağustos" aynı sayılsın diye. */
const kucult = (x) => String(x || "").trim().toLocaleLowerCase("tr");

/** Bir klasör adı ay adı mı? ("Ağustos", "AĞUSTOS", "ağustos" hepsi evet.) */
function ayMi(ad) {
  return AYLAR.some((a) => kucult(a) === kucult(ad));
}
...
async function klasorBulVeyaOlustur(jeton, ad, ustKlasor) {
  ...
  const eslesen = (bulunan.files || []).find((f) => kucult(f.name) === kucult(ad));
...
async function dosyaninAyiniBul(jeton, dosyaParents, markaKoku) {
  const simdikiAy = AYLAR[new Date().getMonth()];
...
export async function hedefKlasoruHazirla({ markaKlasoru, markaAdi, durumAdi, ayAdi }) {
  ...
  try {
    ...
    const ay = ayAdi || AYLAR[new Date().getMonth()];
