...
async function calismaKokunuBul(jeton, klasorId) {
  try {
    const sorgu = encodeURIComponent(
      `'${klasorId}' in parents and mimeType='application/vnd.google-apps.folder' and trashed=false`,
    );
    const r = await getir(
      `https://www.googleapis.com/drive/v3/files?q=${sorgu}&fields=files(id,name)&pageSize=200&supportsAllDrives=true&includeItemsFromAllDrives=true`,
      { headers: { Authorization: `Bearer ${jeton}` } },
    );
    if (!r.ok) return klasorId;
    const d = await r.json();
    const sosyal = (d.files || []).find((f) => adlarAyniMi(f.name, SOSYAL_MEDYA));
    return sosyal ? sosyal.id : klasorId;
  } catch (e) {
    return klasorId;   // bulunamazsa gösterilen klasörle devam — taşıma durmasın
  }
}
...
export async function klasorDurumu(markaKlasoru) {
  ...
  try {
    ...
    const calismaKoku = await calismaKokunuBul(jeton, d.id);
    const indi = calismaKoku !== d.id;
    const yol = ust ? `${ust} / ${d.name}` : d.name;
    return {
      durum: "tamam", klasorAdi: d.name, ustKlasor: ust, yol,
      indi,
      hedefYol: indi ? `${yol} / ${SOSYAL_MEDYA}` : yol,
    };
  } catch (e) {
