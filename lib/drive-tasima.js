...
async function klasorBulVeyaOlustur(jeton, ad, ustKlasor) {
  ...
  const ara = await fetch(`https://www.googleapis.com/drive/v3/files?q=${sorgu}&fields=files(id)&supportsAllDrives=true&includeItemsFromAllDrives=true`, {
    headers: { Authorization: `Bearer ${jeton}` },
  });
  const bulunan = await ara.json();
  if (bulunan.files && bulunan.files.length > 0) return bulunan.files[0].id;

  const olustur = await fetch("https://www.googleapis.com/drive/v3/files?fields=id&supportsAllDrives=true", {
    method: "POST",
...
export async function onaylananiTasi({ dosyaLinki, markaAdi, markaKlasoru, hedefAd = "ONAYLANANLAR" }) {
  ...
  try {
    ...
    const bilgi = await fetch(`https://www.googleapis.com/drive/v3/files/${dosyaId}?fields=parents,name&supportsAllDrives=true`, {
      headers: { Authorization: `Bearer ${jeton}` },
    });
