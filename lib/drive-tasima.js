...
/* GOOGLE JETONU ÖNBELLEĞİ.
 *
 * Jeton bir saat geçerli ama hiçbir yerde saklanmıyordu: her çağrıda RSA-SHA256 imza
 * atılıp Google'a ayrı bir HTTP turu yapılıyordu. Videoda her ileri-geri sarma yeni bir
 * istek demek olduğu için bu bedel her sarmada yeniden ödeniyordu.
 *
 * Modül düzeyinde tutuluyor: sıcak bir fonksiyon örneğinde sonraki istekler aynı jetonu
 * kullanıyor, soğuk başlangıçta zaten yeniden alınıyor.
 *
 * SÜRESİ DOLMADAN 60 SANİYE ÖNCE yenileniyor — tam sınırda kullanmak, isteğin Google'a
 * varana kadar geçen sürede jetonun sönmesi demek olabilir.
 *
 * UÇUŞTAKİ İSTEK PAYLAŞILIYOR: aynı anda gelen beş istek beş ayrı jeton almasın diye
 * ilk çağrının sözü saklanıyor. Hata durumunda söz temizleniyor, yoksa tek bir geçici
 * hata sonraki bütün çağrıları kalıcı olarak zehirlerdi. */
let jetonOnbellek = null;      // { jeton, biter }
let jetonSozu = null;

/* ÖNBELLEK KENDİNİ ONARABİLMELİ. Jeton süresi dolmadan geçersiz kılınabiliyor (anahtar
 * döndürüldü, saat kayması, Google tarafında iptal). Önbellek körü körüne tutulursa
 * istekler süre dolana kadar 401 almaya devam ederdi — önbellekten ÖNCE olmayan bir
 * takılma. Yetkisiz yanıt gelince jeton unutuluyor ve bir kez yeniden deneniyor. */
export function jetonuUnut() { jetonOnbellek = null; jetonSozu = null; }

async function erisimJetonu() {
  const simdi = Math.floor(Date.now() / 1000);
  if (jetonOnbellek && jetonOnbellek.biter - 60 > simdi) return jetonOnbellek.jeton;
  if (jetonSozu) return jetonSozu;
  jetonSozu = jetonUretVeAl()
    .then((sonuc) => { jetonOnbellek = sonuc; jetonSozu = null; return sonuc.jeton; })
    .catch((e) => { jetonSozu = null; throw e; });
  return jetonSozu;
}

async function jetonUretVeAl() {
  ...
  /* Google `expires_in` döndürüyor (tipik 3600). Gelmezse temkinli bir değer. */
  const omur = Number(veri.expires_in) > 0 ? Number(veri.expires_in) : 3000;
  return { jeton: veri.access_token, biter: Math.floor(Date.now() / 1000) + omur };
}
...
async function dosyaninAyKlasoru(jeton, dosyaParents, calismaKoku) {
  ...
  /* YUKARI DOĞRU YÜRÜYÜŞ — dosya ay klasörünün kaç basamak altında olabilir:
   *
   *   <AY>/dosya                     eski düzen
   *   <AY>/<DURUM>/dosya             bugünkü düzen
   *   <AY>/<DURUM>/<KART>/dosya      carousel kart klasörü
   *
   * Eskiden sabit İKİ basamak yürünüyordu. Kart klasörü gelince üçüncü basamak hiç
   * görülmüyor, dosyanın ayı bulunamıyor ve taşıma "ay yok" sanıp İÇİNDE BULUNULAN ayı
   * hazırlıyordu: temmuzda çekilmiş bir karoselin slaytları onaylanınca ağustos
   * klasörüne sıçrardı. Hata vermeden, sessizce. */
  const SINIR = 4;
  const gorulen = new Set();
  let seviye = (dosyaParents || []).filter(Boolean);
  for (let derinlik = 0; derinlik < SINIR && seviye.length > 0; derinlik += 1) {
    const sonraki = [];
    for (const id of seviye) {
      if (gorulen.has(id)) continue;              // döngüsel bağlantıda sonsuza gitmesin
      gorulen.add(id);
      const k = await klasorBilgisi(jeton, id);
      if (!k) continue;
      const bulundu = await ayKlasoruMu(k);
      if (bulundu) return bulundu;
      sonraki.push(...(k.parents || []));
    }
    seviye = sonraki;
  }
  return null;
 ...
 */
export async function videoAkisi(dosyaId, aralik, iptalSinyali) {
  if (!servisHesabiHazirMi()) return { ok: false, sebep: "Drive servis hesabı kurulu değil." };
  const iste = async () => {
    const jeton = await erisimJetonu();
    return fetch(
      ...
      { headers: { Authorization: `Bearer ${jeton}`, ...(aralik ? { Range: aralik } : {}) },
        /* İstemci vazgeçtiğinde Google'dan indirme de duruyor — bağ olmadan ölü
         * indirmeler fonksiyon süresi boyunca sürüyordu. */
        ...(iptalSinyali ? { signal: iptalSinyali } : {}) },
    );
  };
  try {
    let yanit = await iste();
    /* Önbellekteki jeton erken geçersiz kılındıysa TEK KEZ yenilenip deneniyor.
     * Denenmezse istekler jetonun süresi dolana kadar 401 almaya devam ederdi. */
    if (yanit.status === 401) { jetonuUnut(); yanit = await iste(); }
 ...
 */
/**
 * KART KLASÖRÜNÜN KENDİSİNİ TAŞIR — dosyaları tek tek değil.
 *
 * Karosel kartının slaytları zaten bir klasörün içinde. Onları tek tek taşımak üç
 * şeye mal oluyordu: on slayt için on tur Drive çağrısı, hedefte aynı adla YENİ bir
 * klasör açılması ve kaynakta BOŞ bir klasörün kalması. Klasörü taşımak tek çağrı,
 * artık kalıntı yok ve klasörün kimliği korunuyor — birine klasör bağlantısı
 * verildiyse aşama değişince kırılmıyor.
 *
 * Klasörü servis hesabı açtığı için sahibi odur; yeniden ebeveynlemek için ek yetki
 * gerekmiyor (dosyalarda gerekiyordu, onların sahibi OAuth hesabı).
 *
 * Klasör kimliği kayıtta tutulmuyor; kartın dosyalarından biri ipucu olarak veriliyor
 * ve klasör onun ebeveyninden bulunuyor. Bulunamazsa `tasindi: false` döner ve çağıran
 * eski dosya-dosya yola düşer — bu özellik gelmeden önce yüklenmiş kartlar öyle.
 *
 * `icerdekiDosyalar`: klasörle birlikte taşınmış dosya kimlikleri. Çağıran bunları
 * tekrar taşımaya çalışmasın diye döndürülüyor.
 */
export async function kartKlasorunuTasi({ kartKlasoru, markaAdi, markaKlasoru, hedefAd, ipucuDosyaLinki }) {
  if (!kartKlasoru) return { tasindi: false, sebep: "Kart klasörü adı yok." };
  ...
  const ipucuId = driveDosyaIdCikar(ipucuDosyaLinki);
  if (!ipucuId) return { tasindi: false, sebep: "Kart klasörü bulunamadı." };
  ...
  try {
    ...
    if (!gosterilen) return { tasindi: false, sebep: "Bu marka için Drive klasörü tanımlı değil." };
    const markaKoku = await calismaKokunuBul(jeton, gosterilen);

    /* İpucu dosyanın ebeveynlerinden kart klasörünü bul. */
    const dosyaBilgi = await getir(
      `https://www.googleapis.com/drive/v3/files/${ipucuId}?fields=parents&supportsAllDrives=true`,
      { headers: { Authorization: `Bearer ${jeton}` } },
    );
    const dosya = await dosyaBilgi.json();
    if (!dosyaBilgi.ok) return { tasindi: false, sebep: dosya.error?.message || "Dosya bulunamadı." };

    let klasor = null;
    for (const ustId of dosya.parents || []) {
      const ust = await klasorBilgisi(jeton, ustId);
      if (ust && adlarAyniMi(ust.name, kartKlasoru)) { klasor = ust; break; }
    }
    /* Dosya kart klasöründe değil — bu kart özellik gelmeden önce yüklenmiş.
     * Çağıran dosya-dosya taşısın; o yol klasörü hedefte açıp dosyaları içine alıyor. */
    if (!klasor) return { tasindi: false, sebep: "Kart klasörü bulunamadı.", klasorYok: true };

    const ay = (await dosyaninAyKlasoru(jeton, klasor.parents, markaKoku))
            || (await ayKlasoruHazirla(jeton, markaKoku));
    const hedef = await klasorBulVeyaOlustur(jeton, hedefAd, ay.id);
    const yol = `${ay.yol}/${hedefAd}/${kartKlasoru}`;

    const icerik = (await klasorunDosyalari(jeton, klasor.id)).map((f) => f.id);

    if ((klasor.parents || []).includes(hedef)) {
      return { tasindi: false, zatenOrada: true, klasor: yol, icerdekiDosyalar: icerik };
    }

    const eskiler = (klasor.parents || []).join(",");
    const tasi = await getir(
      `https://www.googleapis.com/drive/v3/files/${klasor.id}?addParents=${hedef}${eskiler ? `&removeParents=${eskiler}` : ""}&fields=id,parents&supportsAllDrives=true`,
      { method: "PATCH", headers: { Authorization: `Bearer ${jeton}`, "Content-Type": "application/json" }, body: "{}" },
    );
    const sonuc = await tasi.json();
    if (!tasi.ok) return { tasindi: false, sebep: sonuc.error?.message || "Klasör taşınamadı." };
    return { tasindi: true, klasor: yol, klasorId: klasor.id, icerdekiDosyalar: icerik };
  } catch (e) {
...
/**
 * KART KLASÖRÜ BOŞALDIYSA ÇÖPE ATAR.
 *
 * Karosel kartının slaytları silinince klasör Drive'da boş duruyordu; ONAY BEKLEYENLER
 * birkaç ayda adı olup içi olmayan klasörlerle doluyor ve gerçek işler arasında
 * kayboluyordu.
 *
 * ÇÖPE — kalıcı silmez. Yanlışlıkla silinen bir gönderi Drive'ın çöp kutusunda 30 gün
 * duruyor ve geri alınabiliyor; kalıcı silme geri dönüşü olmayan bir işlem ve bu
 * projede yasak.
 *
 * SADECE GERÇEKTEN BOŞSA. İçinde tek bir dosya ya da alt klasör kalmışsa dokunulmuyor —
 * "boş sanıp doluyu atmak" bu temizliğin yapabileceği en kötü şey.
 *
 * Klasörü servis hesabı açtığı için sahibi odur; çöpe atmak sahiplik istiyor ve burada
 * var. (Dosyalarda yok — onların sahibi OAuth hesabı, o yüzden dosya silme ayrı yoldan.)
 *
 * İpucu olarak SİLİNMİŞ dosyanın kimliği veriliyor: çöpe atılmış dosyanın ebeveyni hâlâ
 * okunabiliyor, klasör oradan bulunuyor. Klasör kimliğini kayıtta tutmaya gerek kalmıyor.
 */
export async function bosaldiysaKartKlasorunuCopeAt({ kartKlasoru, ipucuDosyaId }) {
  if (!kartKlasoru || !ipucuDosyaId) return { silindi: false, sebep: "Kart klasörü belirlenemedi." };
  if (!process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL || !process.env.GOOGLE_PRIVATE_KEY) {
    return { silindi: false, sebep: "Drive kimlik bilgileri tanımlı değil." };
  }
  try {
    const jeton = await erisimJetonu();
    const r = await getir(
      `https://www.googleapis.com/drive/v3/files/${ipucuDosyaId}?fields=parents&supportsAllDrives=true`,
      { headers: { Authorization: `Bearer ${jeton}` } },
    );
    const dosya = await r.json();
    if (!r.ok) return { silindi: false, sebep: dosya.error?.message || "Dosya bulunamadı." };
    ...
    if (!klasor) return { silindi: false, klasorYok: true, sebep: "Kart klasörü bulunamadı." };

    /* HER ŞEY sayılıyor — yalnızca dosyalar değil. İçinde bir alt klasör kalmışsa bu
     * klasör boş değildir; kullanıcının elle koyduğu bir şey olabilir. */
    const kalan = await klasorunIcerigi(jeton, klasor.id);
    if (kalan.length > 0) {
      return { silindi: false, doluKaldi: true, kalanSayisi: kalan.length };
    }

    const copeAt = await getir(
      `https://www.googleapis.com/drive/v3/files/${klasor.id}?fields=id,trashed&supportsAllDrives=true`,
      {
        method: "PATCH",
        headers: { Authorization: `Bearer ${jeton}`, "Content-Type": "application/json" },
        body: JSON.stringify({ trashed: true }),
    ...
    const sonuc = await copeAt.json();
    if (!copeAt.ok) return { silindi: false, sebep: sonuc.error?.message || "Klasör çöpe atılamadı." };
    return { silindi: true, klasorAdi: klasor.name };
  } catch (e) {
    return { silindi: false, sebep: String(e.message || e) };
  }
}

/**
 * DRIVE SAĞLIK KONTROLÜ — SALT OKUNUR.
 *
 * "Bugün çalışıyor" yetmiyor: Drive tarafı sessizce bozulabiliyor. Klasör elle
 * silinebiliyor, OAuth jetonu süresi dolabiliyor, servis hesabının klasördeki yetkisi
 * kaldırılabiliyor. Hiçbiri hata vermiyor — dosya "taşınmadı" diye karta bir not
 * düşüyor ve kimse fark etmiyor.
 *
 * CANLI DRIVE'DA HİÇBİR ŞEY OLUŞTURULMAZ. Ne deneme klasörü, ne deneme dosyası, ne
 * silme. Yalnızca okuma yapılır. Bu yüzden bazı yetenekler doğrudan denenemez —
 * onlar için Drive'ın `capabilities` alanı okunuyor: Google, bir klasöre çocuk
 * ekleyip ekleyemeyeceğimizi kendisi söylüyor. Okunamayan yetenek "doğrulanamadı"
 * olarak bildirilir; ÜRETİM DRIVE'INDA DENEME YAPILMAZ.
 *
 * Her adım bağımsız: biri hata verse de kalanlar denenir ve rapor eksik kalmaz.
 */
export async function driveSagligi({ markaKlasoru, oauthJetonuAl } = {}) {
  const rapor = {
    servisHesabiTanimli: Boolean(process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL && process.env.GOOGLE_PRIVATE_KEY),
    oauthTanimli: Boolean(process.env.GOOGLE_OAUTH_CLIENT_ID
      && process.env.GOOGLE_OAUTH_CLIENT_SECRET && process.env.GOOGLE_OAUTH_REFRESH_TOKEN),
    kokKlasorTanimli: Boolean(process.env.DRIVE_ONAY_KLASOR_ID || driveKlasorIdCikar(markaKlasoru)),
    adimlar: [],
  };
  const ekle = (ad, durum, not) => rapor.adimlar.push({ ad, durum, not: not || "" });

  if (!rapor.servisHesabiTanimli) {
    ekle("Servis hesabı kimliği", "eksik", "GOOGLE_SERVICE_ACCOUNT_EMAIL / GOOGLE_PRIVATE_KEY tanımlı değil");
  }
  if (!rapor.oauthTanimli) {
    ekle("OAuth kimliği", "eksik", "Dosya yükleme çalışmaz — servis hesabı yükleyemez");
  }

  const hedefKok = driveKlasorIdCikar(markaKlasoru) || process.env.DRIVE_ONAY_KLASOR_ID || null;
  if (!hedefKok) {
    ekle("Ana klasör", "eksik", "Ne marka klasörü ne DRIVE_ONAY_KLASOR_ID verilmiş");
  }

  /* SERVİS HESABI TARAFI — okuma. */
  if (rapor.servisHesabiTanimli && hedefKok) {
    try {
      const jeton = await erisimJetonu();
      ekle("Servis hesabı jetonu", "tamam");

      const r = await getir(
        `https://www.googleapis.com/drive/v3/files/${hedefKok}?fields=id,name,trashed,capabilities(canAddChildren,canListChildren)&supportsAllDrives=true`,
        { headers: { Authorization: `Bearer ${jeton}` } },
      );
      const k = await r.json();
      if (!r.ok) {
        ekle("Ana klasöre erişim", "hata", k.error?.message || "Klasör okunamadı");
      } else if (k.trashed) {
        ekle("Ana klasöre erişim", "hata", `"${k.name}" ÇÖP KUTUSUNDA — dosyalar oraya taşınamaz`);
      } else {
        ekle("Ana klasöre erişim", "tamam", k.name || "");

        /* YAZMA YETENEĞİ DENENMEDEN OKUNUYOR. Google `capabilities` alanında
         * klasöre çocuk ekleyip ekleyemeyeceğimizi söylüyor; deneme klasörü
         * açmaya gerek yok. Alan gelmezse "doğrulanamadı". */
        const yetki = k.capabilities || {};
        if (yetki.canAddChildren === true) {
          ekle("Servis hesabı klasör açabiliyor", "tamam", "Drive yetki bilgisinden okundu");
        } else if (yetki.canAddChildren === false) {
          ekle("Servis hesabı klasör açabiliyor", "hata",
            "Bu klasörde yazma yetkisi yok — klasörü servis hesabıyla paylaş");
        } else {
          ekle("Servis hesabı klasör açabiliyor", "dogrulanamadi",
            "Drive yetki bilgisini vermedi; üretim klasöründe deneme yapılmadı");
        }

        try {
          const alt = await altKlasorler(jeton, hedefKok);
          ekle("Klasör listeleme", "tamam", `${alt.length} alt klasör görüldü`);
        } catch (e) {
          ekle("Klasör listeleme", "hata", String(e.message || e));
        }
      }
    } catch (e) {
      ekle("Servis hesabı jetonu", "hata", String(e.message || e));
    }
  }

  /* OAUTH TARAFI — okuma. Jeton alınabiliyorsa ve hesabın alanı varsa yükleme
   * çalışır. Deneme dosyası YÜKLENMEZ. */
  if (rapor.oauthTanimli && typeof oauthJetonuAl === "function") {
    try {
      const jeton = await oauthJetonuAl();
      const r = await getir(
      ...
      const d = await r.json();
      if (!r.ok) {
        ekle("OAuth yükleme kimliği", "hata", d.error?.message || "Jeton geçersiz olabilir");
      } else {
        const kota = d.storageQuota || {};
        const sinir = Number(kota.limit || 0);
        const kullanim = Number(kota.usage || 0);
        const kalan = sinir > 0 ? sinir - kullanim : null;
        ekle("OAuth yükleme kimliği", "tamam", d.user?.emailAddress || "");
        if (kalan !== null && kalan <= 0) {
          ekle("Drive alanı", "hata", "Depolama dolu — yeni dosya yüklenemez");
        } else if (kalan !== null) {
          ekle("Drive alanı", "tamam", `${Math.round(kalan / 1e9)} GB boş`);
        } else {
          ekle("Drive alanı", "dogrulanamadi", "Kota bilgisi okunamadı");
        }
      }
    } catch (e) {
      ekle("OAuth yükleme kimliği", "hata", String(e.message || e));
    }
  }

  rapor.hataSayisi = rapor.adimlar.filter((x) => x.durum === "hata" || x.durum === "eksik").length;
  rapor.dogrulanamayan = rapor.adimlar.filter((x) => x.durum === "dogrulanamadi").length;
  return rapor;
}

/**
 * MARKANIN ONAYLANANLAR KLASÖRÜNDEKİ DOSYALAR — SALT OKUNUR.
 *
 * "Stok sayısı fazla görünüyor — eksik kart mı var?" sorusunun Drive tarafındaki
 * yarısı. Kartlar yükledikleri her dosyanın kimliğini saklıyor; buradan gelen liste
 * onunla karşılaştırılınca "Drive'da var ama kartı yok" dosyalar ortaya çıkıyor.
 *
 * HİÇBİR ŞEY YAZMAZ. Ne klasör açar, ne dosya taşır, ne siler. Yalnızca listeler.
 *
 * ÇAĞRI BÜTÇESİ VAR. Bir markanın on iki ay klasörü olabiliyor ve her ay için iki
 * listeleme gerekiyor; sınırsız gezmek Vercel'in fonksiyon süresini aşardı. Bütçe
 * dolarsa liste EKSİK döner ve bunu `tamamlanmadi` ile SÖYLER — sessizce kesilen
 * bir liste "her şeyi gördük" sanılır ve yanlış karar verdirir.
 */
export async function markaninDriveDosyalari({ markaKlasoru, markaAdi, cagriButcesi = 90, durumlar = null }) {
  const markaHedef = driveKlasorIdCikar(markaKlasoru);
  if (!markaHedef && !driveTasimaHazirMi()) return { ok: false, sebep: "Drive taşıma kurulu değil." };
  if (!process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL || !process.env.GOOGLE_PRIVATE_KEY) {
    return { ok: false, sebep: "Drive kimlik bilgileri tanımlı değil." };
  }

  let butce = cagriButcesi;
  const harca = () => (butce-- > 0);

  try {
    const jeton = await erisimJetonu();
    const kok = process.env.DRIVE_ONAY_KLASOR_ID;
    /* KLASÖR ARANIR, AÇILMAZ. Burada bir süre `klasorBulVeyaOlustur` çağrılıyordu:
     * marka klasörü tanımsızsa tarama, "bakıyorum" derken ortak klasörün altında
     * marka adıyla YENİ bir klasör açıyordu. Teşhis aracı üretim Drive'ında hiçbir
     * şey yazmamalı — t85 bunu ölçüyor. */
    let gosterilen = markaHedef;
    if (!gosterilen && markaAdi && kok) {
      const eslesen = (await altKlasorler(jeton, kok)).find((f) => adlarAyniMi(f.name, markaAdi));
      gosterilen = eslesen ? eslesen.id : null;
    }
    if (!gosterilen) {
      return { ok: false, sebep: "Bu marka için Drive klasörü bulunamadı — müşteri kartında Drive onay klasörü tanımlı değil." };
    }
    const markaKoku = await calismaKokunuBul(jeton, gosterilen);

    /* AY klasörleri kökün altında ya da bir YIL klasörünün altında olabiliyor —
     * iki yerleşim de destekleniyor (taşıma yolu da böyle davranıyor). */
    const aylar = [];
    const ustler = await altKlasorler(jeton, markaKoku);
    for (const k of ustler) {
      if (ayMi(k.name)) { aylar.push({ id: k.id, yol: k.name }); continue; }
      if (yilKlasoruMu(k.name) && harca()) {
        const icAylar = await altKlasorler(jeton, k.id);
        icAylar.filter((x) => ayMi(x.name)).forEach((x) => aylar.push({ id: x.id, yol: `${k.name}/${x.name}` }));
      }
    }
    /* En yeni aylar önce: bütçe dolarsa güncel dönem taranmış olur. */
    aylar.sort((a, b) => b.yol.localeCompare(a.yol, "tr"));

    /* HANGİ DURUM KLASÖRLERİ. Varsayılan: üçü birden. Stok yalnızca ONAYLANANLAR'a
     * bakıyor ama "onay bekleyende ne var" sorusu operasyonun günlük sorusu — ikisini
     * ayrı tarama yapmak çağrı sayısını iki katına çıkarırdı, tek geçişte toplanıyor. */
    const bakilacak = Object.entries(DURUM_KLASORLERI)
      .filter(([anahtar]) => !durumlar || durumlar.includes(anahtar));

    const dosyalar = [];
    let tamamlanmadi = false;
    const bakilanAylar = [];

    dis: for (const ay of aylar) {
      if (!harca()) { tamamlanmadi = true; break; }
      const altlar = await altKlasorler(jeton, ay.id);
      let ayaBakildi = false;

      for (const [durumAnahtari, durumAdi] of bakilacak) {
        const durumKlasoru = altlar.find((x) => adlarAyniMi(x.name, durumAdi));
        if (!durumKlasoru) continue;
        ayaBakildi = true;

        if (!harca()) { tamamlanmadi = true; break dis; }
        (await klasorunDosyalari(jeton, durumKlasoru.id)).forEach((f) =>
          dosyalar.push({ id: f.id, ad: f.name, durum: durumAnahtari, ay: ay.yol, klasor: `${ay.yol}/${durumAdi}` }));

        /* KART KLASÖRLERİ (Carousel). Slaytlar durum klasörünün doğrudan altında
         * değil, kartın kendi klasöründe duruyor — atlanırsa hepsi "kartsız" sanılır. */
        if (!harca()) { tamamlanmadi = true; break dis; }
        const kartKlasorleri = await altKlasorler(jeton, durumKlasoru.id);
        for (const kk of kartKlasorleri) {
          if (!harca()) { tamamlanmadi = true; break dis; }
          (await klasorunDosyalari(jeton, kk.id)).forEach((f) =>
            dosyalar.push({ id: f.id, ad: f.name, durum: durumAnahtari, ay: ay.yol,
              klasor: `${ay.yol}/${durumAdi}/${kk.name}`, kartKlasoru: kk.name }));
        }
      }
      if (ayaBakildi) bakilanAylar.push(ay.yol);
    }

    /* AY KLASÖRÜ HİÇ BULUNAMADIYSA bu "içerik yok" DEĞİLDİR — yapı beklenenden farklı
     * (klasör adı ay biçiminde değil, bağlantı yanlış klasörü gösteriyor, erişim yok).
     * Ölçüldü: bu ayrım olmadan tarama "0 dosya, tamamlandı" diyor ve Drive otoriteli
     * stok bütün sayıları SIFIRLIYOR. Sessiz veri kaybı. */
    return { ok: true, dosyalar, bakilanAylar, toplamAy: aylar.length, tamamlanmadi,
      ayBulunamadi: aylar.length === 0 };
  } catch (e) {
    return { ok: false, sebep: String(e.message || e) };
  }
}
...
/**
 * YALNIZCA ONAYLANANLAR — eski çağrılar için ince sarmalayıcı.
 * Stok otoritesi bu klasör olduğu için ayrı bir adı hak ediyor.
 */
export async function markaninOnaylananDosyalari(secenekler) {
  return markaninDriveDosyalari({ ...secenekler, durumlar: ["onaylanan"] });
}


/** Bir klasörün içindeki HER ŞEY — dosya, alt klasör, hepsi. Boşluk kontrolü için. */
async function klasorunIcerigi(jeton, klasorId) {
  const sorgu = encodeURIComponent(`'${klasorId}' in parents and trashed=false`);
  const r = await getir(
    `https://www.googleapis.com/drive/v3/files?q=${sorgu}&fields=files(id)&pageSize=10&supportsAllDrives=true&includeItemsFromAllDrives=true`,
    { headers: { Authorization: `Bearer ${jeton}` } },
  );
  const d = await r.json();
  /* Listeleme BAŞARISIZ olursa "boş" sayılmaz — hata anında klasörü atmak, dolu bir
   * klasörü çöpe göndermek demek olurdu. Bir öğe varmış gibi davranılıyor. */
  if (!r.ok) return [{ id: "okunamadi" }];
  return d.files || [];
}

/** Bir klasörün içindeki dosyalar (klasörler hariç). */
async function klasorunDosyalari(jeton, klasorId) {
  const sorgu = encodeURIComponent(`'${klasorId}' in parents and trashed=false`);
  try {
    const r = await getir(
      `https://www.googleapis.com/drive/v3/files?q=${sorgu}&fields=files(id,name,mimeType)&pageSize=200&supportsAllDrives=true&includeItemsFromAllDrives=true`,
      { headers: { Authorization: `Bearer ${jeton}` } },
    );
    const d = await r.json();
    if (!r.ok) return [];
    return (d.files || []).filter((f) => f.mimeType !== "application/vnd.google-apps.folder");
  } catch (e) {
    return [];
  }
}

export async function onaylananiTasi({ dosyaLinki, markaAdi, markaKlasoru, hedefAd = DURUM_KLASORLERI.onaylanan, kartKlasoru = null }) {
  ...
  try {
    ...
    const ay = (await dosyaninAyKlasoru(jeton, dosya.parents, markaKoku))
            || (await ayKlasoruHazirla(jeton, markaKoku));
    const durumKlasoru = await klasorBulVeyaOlustur(jeton, hedefAd, ay.id);

    /* KART KLASÖRÜ — çok dosyalı gönderi durum klasörüne dağılmasın.
     *
     * Carousel bir kartta sekiz on slayt olabiliyor; hepsi ONAYLANANLAR'a tek tek
     * düştüğünde hangi karenin hangi gönderiye ait olduğu ayırt edilemiyor. Ad
     * çağırandan geliyor (`kartKlasorAdi`), burası yalnızca bir alt basamak açıyor.
     * Verilmezse dosya bugünkü gibi doğrudan durum klasörüne gider. */
    const hedef = kartKlasoru
      ? await klasorBulVeyaOlustur(jeton, kartKlasoru, durumKlasoru)
      : durumKlasoru;
    const yol = kartKlasoru ? `${ay.yol}/${hedefAd}/${kartKlasoru}` : `${ay.yol}/${hedefAd}`;
 ...
 */
export async function hedefKlasoruHazirla({ markaKlasoru, markaAdi, durumAdi, kartKlasoru = null }) {
  const markaHedef = driveKlasorIdCikar(markaKlasoru);
  if (!process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL || !process.env.GOOGLE_PRIVATE_KEY) {
    return { ok: false, sebep: "Drive kimlik bilgileri tanımlı değil." };
  }
  ...
  try {
    ...
    const durum = await klasorBulVeyaOlustur(jeton, durumAdi, ay.id);

    /* KART KLASÖRÜ — dosya BURAYA yükleniyor, sonradan taşınmıyor.
     *
     * Taşıma yoluna kart klasörü eklenmişti ama yükleme AYRI bir yol: dosya doğrudan
     * hedefe yükleniyor (servis hesabı yükleyemediği için "önce yükle sonra taşı"
     * yapılamıyor). Bu yüzden karosel slaytları aşama değişene kadar ONAY
     * BEKLEYENLER'de tek tek duruyordu — ölçüldü, kullanıcı ekranda gördü.
     *
     * Klasörü servis hesabı açıyor, dosyayı OAuth hesabı onun içine yüklüyor; bu
     * zaten durum klasörlerinde çalışan düzenin bir basamak derini. */
    const hedef = kartKlasoru ? await klasorBulVeyaOlustur(jeton, kartKlasoru, durum) : durum;
    const yol = kartKlasoru ? `${ay.yol}/${durumAdi}/${kartKlasoru}` : `${ay.yol}/${durumAdi}`;
    return { ok: true, klasorId: hedef, yol, ayAdi: ay.yol };
  } catch (e) {
    return { ok: false, sebep: String(e.message || e) };
  }
}
...
/**
 * DOSYANIN İLK BAYTLARI — "fast start" teşhisi için.
 *
 * Yalnızca birkaç kilobayt okunuyor: oynatma bilgisinin (`moov`) başta mı sonda mı
 * olduğunu anlamaya bu yetiyor. Başarısız olursa null döner — teşhis yapılamaması
 * videoyu engellememeli.
 */
/** Dosyanın ilk baytları + TOPLAM BOYUTU.
 *
 * Boyut için ayrı bir istek atılmıyor: aralık isteğinin yanıtı zaten
 * `Content-Range: bytes 0-4095/67108864` diye toplamı taşıyor. Ayrı bir
 * `fields=size` çağrısı her video açılışına bir Google turu daha eklerdi. */
export async function dosyaBasligi(dosyaId, bayt = 4096) {
  if (!servisHesabiHazirMi()) return null;
  try {
    const jeton = await erisimJetonu();
    const yanit = await fetch(
      `https://www.googleapis.com/drive/v3/files/${dosyaId}?alt=media&supportsAllDrives=true`,
      { headers: { Authorization: `Bearer ${jeton}`, Range: `bytes=0-${bayt - 1}` } },
    );
    if (!yanit.ok && yanit.status !== 206) return null;
    const tampon = await yanit.arrayBuffer();
    const aralik = yanit.headers.get("content-range");
    const eslesme = aralik ? /\/(\d+)\s*$/.exec(aralik) : null;
    return { baytlar: new Uint8Array(tampon), toplamBoyut: eslesme ? Number(eslesme[1]) : 0 };
  } catch (e) {
    return null;
  }
}
