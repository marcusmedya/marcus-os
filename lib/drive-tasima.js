 ...
 *
 * Karttan bir slayt silindiğinde Drive'daki karşılığı da gitmeli, yoksa klasör kimsenin
 * kullanmadığı dosyalarla dolar. Ama KALICI silme yanlış olurdu: yanlış slaydı silmek bir
 * tıklık iş, geri getirmek imkânsız. Çöpe atılan dosya Drive'da 30 gün duruyor ve
 * "Çöp Kutusu"ndan geri alınabiliyor.
 *
 * Servis hesabı kullanılıyor: uygulamanın YÜKLEMEDİĞİ dosyalar da (elle yapıştırılmış eski
 * bağlantılar) marka klasöründe duruyor ve OAuth'un dar `drive.file` kapsamı onları göremiyor.
 * Servis hesabının o klasörde düzenleme yetkisi var.
 */
