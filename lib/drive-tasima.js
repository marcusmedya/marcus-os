 ...
 */
export async function markaninOnaylananDosyalari({ markaKlasoru, markaAdi, cagriButcesi = 60 }) {
  const markaHedef = driveKlasorIdCikar(markaKlasoru);
  ...
  try {
    ...
    for (const ay of aylar) {
      ...
      const onaylanan = altlar.find((x) => adlarAyniMi(x.name, DURUM_KLASORLERI.onaylanan));
      if (!onaylanan) continue;
      bakilanAylar.push(ay.yol);

      if (!harca()) { tamamlanmadi = true; break; }
      (await klasorunDosyalari(jeton, onaylanan.id)).forEach((f) =>
        dosyalar.push({ id: f.id, ad: f.name, klasor: `${ay.yol}/${DURUM_KLASORLERI.onaylanan}` }));
      ...
      if (!harca()) { tamamlanmadi = true; break; }
      const kartKlasorleri = await altKlasorler(jeton, onaylanan.id);
      for (const kk of kartKlasorleri) {
        if (!harca()) { tamamlanmadi = true; break; }
        (await klasorunDosyalari(jeton, kk.id)).forEach((f) =>
          dosyalar.push({ id: f.id, ad: f.name, klasor: `${ay.yol}/${DURUM_KLASORLERI.onaylanan}/${kk.name}` }));
      }
      if (tamamlanmadi) break;
    }
