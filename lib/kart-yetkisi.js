import { ILK_ASAMA } from "./asamalar.js";
...
export function izinsizKartDegisiklikleriniGeriAl(eskiIsler, yeniIsler, izinler) {
  ...
  yeniIsler.forEach((yeni) => {
    ...
    if (!eski) {
      ...
      /* KART DOĞRUDAN ONAY AŞAMASINDA AÇILAMAZ — onaylama yetkisi yoksa.
       *
       * Denetim yalnızca AŞAMA GEÇİŞİNE bakıyordu: var olan bir kartı onaya almak yetki
       * istiyordu ama kartı en baştan "Onaylandı" olarak AÇMAK hiç sorulmuyordu. Sonuç,
       * onaylama yetkisi olmayan personelin tek adımda stok üretebilmesiydi — sınırın
       * etrafından dolaşan bir yol. Kart açılmaya devam ediyor, yalnızca aşaması akışın
       * başına çekiliyor: kullanıcının emeği çöpe atılmıyor, yetki sınırı korunuyor. */
      if (ONAY_ASAMALARI.includes(yeni.asama) && !onaylayabilir) {
        sonuc.push({ ...yeni, asama: ILK_ASAMA(yeni.kategori) });
        geriAlinanlar.push({ id: yeni.id, marka: yeni.marka, sebep: "kartOnaylama" });
        return;
      }
      sonuc.push(yeni);
      return;
