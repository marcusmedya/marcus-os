...
/**
 * KARTIN ŞUBE KAPSAMI KAYIP MI?
 *
 * Kart bir şubeye kilitliyken o şube silinirse kilit anlamsız kalır. İki kötü ihtimal
 * var ve ikisi de sessiz:
 *
 *   1. Kimliği karttan çıkarıp listeyi boşaltmak → `sadeceSubeler: []` "marka geneli"
 *      demek olduğu için "yalnızca Lara için hazırlanmış içerik" bir anda BÜTÜN
 *      şubelerde kullanılabilir hale gelir. Ölçüldü: iki şubeli markada Lara silinince
 *      kart Merkez'de kullanılabilir oluyordu.
 *   2. Kimliği bırakıp hiçbir şey söylememek → kart her şubenin seçicisinden kaybolur,
 *      hata da vermez.
 *
 * Seçilen yol: kimlik BIRAKILIR (kart kullanılamaz kalır, yanlış şubede paylaşılmaz)
 * ama bu durum ADI OLAN bir hal olur ve arayüzde gösterilir. Kullanıcı kapsamı yeniden
 * seçince kart normale döner.
 *
 * Türetilmiş — belgede saklanan yeni bir alan YOK. "Kapsam dolu ama karşılığı olan hiç
 * şube kalmamış" demek.
 */
export function kapsamiKayipMi(is, subeler, clientId) {
  const secili = Array.isArray(is && is.sadeceSubeler) ? is.sadeceSubeler.map(String) : [];
  if (secili.length === 0) return false;              // marka geneli — kapsam yok, kayıp da yok
  return kullanabilenSubeler(is, subeler, clientId).length === 0;
}

/**
 * Karttaki karşılığı olmayan şube kimlikleri. Kapsam yeniden seçilirken temizlenir;
 * eski kimlik listede kalırsa kullanıcı şube seçse bile kart kullanılamaz kalırdı.
 */
export function gecersizSubeKimlikleri(is, subeler, clientId) {
  const secili = Array.isArray(is && is.sadeceSubeler) ? is.sadeceSubeler.map(String) : [];
  if (secili.length === 0) return [];
  const gecerli = new Set(markaninSubeleri(subeler, clientId).map((s) => String(s.id)));
  return secili.filter((x) => !gecerli.has(x));
}
